...
while preserving its intent. Before applying, in order:
  ...
  2. VALIDATION GATE: the rewrite MUST still contain all five-law markers - a
     ...
     a rejected finding, and move on.
  3. APPLY via the CLI (sanctioned path), do NOT hand-edit registry.json:
       claudecron add <id> --prompt-file <new-prompt-tmpfile>
     (re-add updates the prompt; all other fields - interval, cwd, tools, backend,
     add_dirs, enabled - are preserved automatically when you omit their flags).
  4. SELF-EDIT RULE: if the candidate is self-improve itself, make the smallest
     viable change and keep all five laws verbatim.

# STEP 4 - REPORT (only if something was applied or rejected)
Print ONE line per changed or rejected loop: what was noisy/missed/errored, what
changed (or was rejected), and the backup path. If you have been granted a
notification tool by this loop's allowed_tools (for example a chat or webhook
tool), also send that same summary through it. If nothing was applied and nothing
was rejected, report NOTHING (print exactly: noop).
